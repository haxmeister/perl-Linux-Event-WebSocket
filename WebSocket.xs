...
static void
lews_bq_push_event(AV *events, IV opcode, const char *data, size_t size)
{
    AV *event = newAV();
    av_push(event, newSViv(opcode));
    av_push(event, newSVpvn(data == NULL ? "" : data, size));
    av_push(events, newRV_noinc((SV *)event));
}
...
PPCODE:
    ...
    while ((msg = bqws_recv(state->ws)) != NULL) {
        switch (msg->type) {
        case BQWS_MSG_TEXT:
            lews_bq_push_event(events, 1, msg->data, msg->size);
            break;
        case BQWS_MSG_BINARY:
            lews_bq_push_event(events, 2, msg->data, msg->size);
            break;
        case BQWS_MSG_CONTROL_CLOSE:
            lews_bq_push_event(events, 8, msg->data, msg->size);
            break;
    ...
    EXTEND(SP, 3);
    PUSHs(sv_2mortal(newRV_noinc((SV *)events)));
