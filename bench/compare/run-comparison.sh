...
run_matrix() {
    ...
    for type in binary text; do
        run_case "$name" "$port" "$type" 64 1 32
        run_case "$name" "$port" "$type" 1024 1 32
        run_case "$name" "$port" "$type" 16384 1 32
        run_case "$name" "$port" "$type" 64 10 8
        run_case "$name" "$port" "$type" 64 100 8
    done
}

start_server linux_event 9101 perl -Iblib/lib bench/compare/servers/linux-event.pl
run_matrix linux_event 9101

start_server mojolicious 9102 perl bench/compare/servers/mojo.pl
run_matrix mojolicious 9102

start_server node_ws 9103 node bench/compare/servers/node.mjs
run_matrix node_ws 9103

start_server gorilla 9104 env GOMAXPROCS=1 /tmp/gorilla-websocket-bench
run_matrix gorilla 9104
